...
class Extension extends Anternet.Extension {
  ...
  onBroadcast(rid, args, rinfo) {
    if (args.length < 1 || !(args[0] instanceof Buffer) || args[0].length < MSG_HEADER_LENGTH) {
      this.anternet.error(Anternet.Errors.BAD_REQUEST, rid, 'Unknown broadcast format', rinfo.port, rinfo.address);
      return;
    }
    ...
    try {
      ...
      this.anternet.error(Anternet.Errors.BAD_REQUEST, rid, 'Invalid msg', rinfo.port, rinfo.address);
      return;
    }

    if (!secp256k1.verify(hash, signature, publicKey)) {
      this.anternet.error(Anternet.Errors.BAD_REQUEST, rid, 'Invalid signature', rinfo.port, rinfo.address);
      return;
    }
    ...
    if (!channel) {
      this.anternet.error(Anternet.Errors.NOT_ACCEPTABLE, rid, 'Unknown channel', rinfo.port, rinfo.address);
      return;
    }
